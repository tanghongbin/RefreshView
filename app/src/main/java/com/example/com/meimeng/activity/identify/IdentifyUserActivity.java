...
//    TextView describe1Text;
    ...
    private void initView() {
        ...
        describeImageView.setImageResource(R.drawable.icon_identity);
//        describe1Text.setText("身份认证");
//        describe2Text.setText("提供真实有效的身份证正面照，验证真实身份与实名信息。上传的认证资料将经过验证系统加密处理，以保证个人信息不被泄露，请放心上传。");
        describe2Text.setText("提供真实有效的身份证正面照，验证真实身份与实名信息。上传的认证资料将经过验证系统加密处理，以保证个人信息不被泄露，请放心上传。");
        leftArrowImageView.setVisibility(View.VISIBLE);
        bowArrowImageView.setVisibility(View.GONE);
        Utils.readBitMap(IdentifyUserActivity.this, mBgExample, R.raw.identify_user);
        initImageView();
    }
    ...
    private void initImageView() {
        //如果有网
        if (InternetUtils.isNetworkConnected(IdentifyUserActivity.this)) {
                        ...
                        .subscribe(new Action1() {
                            ...
                            public void call(Object o) {
                                ...
                                if (certificateStatusBean.isSuccess()) {
                                    ...
                                    DialogUtils.setDialog(IdentifyUserActivity.this, certificateStatusBean.getError());
    ...
    private void initWedget(CertificateStatusBean certificateStatusBean) {
        if (certificateStatusBean.getParam().getStatus().getIdentityPicId() != null) {
            //已经上传了照片，正在审核中，也不能再上传照片了，点击无效
            List<Long> pics=certificateStatusBean.getParam().getStatus().getIdentityPicId() ;
            ...
            //判断有几张照片
            switch (certificateStatusBean.getParam().getStatus().getIdentityPicId().size()) {
                case 1:
                    sureText.setVisibility(View.VISIBLE);
                    mIsShow.setVisibility(View.GONE);
                    uploadLayout.setVisibility(View.VISIBLE);
                    if (certificateStatusBean.getParam().getStatus().getIdeStatus() == HASCERTIFICATE) {
                        describe3Text.setText("审核通过");
                        //认证通过了就不能再上传，点击无效
                        setButton(certificateStatusBean);
                        picId_0 = certificateStatusBean.getParam().getStatus().getIdentityPicId().get(0);
                    ...
                    } else if (certificateStatusBean.getParam().getStatus().getIdeStatus() == UNCERTIFICATE) {
                ...
                case 2:
                    ...
                    if (certificateStatusBean.getParam().getStatus().getIdeStatus() == HASCERTIFICATE) {
                        ...
                        setButton(certificateStatusBean);
                        picId_0 = certificateStatusBean.getParam().getStatus().getIdentityPicId().get(0);
                        picId_1 = certificateStatusBean.getParam().getStatus().getIdentityPicId().get(1);
                        InternetUtils.getPicIntoView(300, 100, bg_first, picId_0);
                ...
                default:
                    ...
                    if (certificateStatusBean.getParam().getStatus().getIdeStatus() == HASCERTIFICATE) {
                        ...
                        picId_0 = certificateStatusBean.getParam().getStatus().getIdentityPicId().get(0);
                        picId_1 = certificateStatusBean.getParam().getStatus().getIdentityPicId().get(1);
                        picId_2 = certificateStatusBean.getParam().getStatus().getIdentityPicId().get(2);
                        InternetUtils.getPicIntoView(300, 100, bg_first, picId_0);
                        InternetUtils.getPicIntoView(300, 100, bg_second, picId_1);
    ...
    public void identifyUser(ArrayList<Long> picId) {
        try {
            //获取用户的uid,和token
            if (TextUtils.isEmpty( Utils.getUserId() )) {
                return;
            ...
            for (int i = 0; i < picId.size(); i++) {
                jsonStringer.value(picId.get(i));
            }
                        ...
                        public void call(String s) {
                            ...
                            if (basebeanJson.isSuccess()) {
                                ...
                                if (basebeanJson.getError().equals("未登录")) {
                                    DialogUtils.setDialog(IdentifyUserActivity.this, basebeanJson.getError());
                                }
                                displayAfterUploadfailed();
                                Toast.makeText(IdentifyUserActivity.this, basebeanJson.getError(), Toast.LENGTH_SHORT).show();
                            }
                        }
                    }, new Action1<Throwable>() {
                        @Override
    ...
    void sureListener() {
        if (picIdList.size() !=0l) {
            identifyUser(picIdList);
        }
    }
    ...
    private void setSelectPictureDialog() {
        if (selectPictureDialog == null) {
            selectPictureDialog = new SelectPictureDialog(IdentifyUserActivity.this, R.style.loading_dialog);
        }
        ...
        if (data != null) {
            switch (requestCode) {
                ...
                case SelectPictureDialog.CHOOSE_BIG_PICTURE:
                    String path = ImageUtil.getPath(IdentifyUserActivity.this, SelectPictureDialog.imageUri);
                    Bitmap bitmap = ImageUtil.getBitmapFromPath(path);
                    bg_first.setImageBitmap(bitmap);
                ...
                case CAMEAR_CROP_PICTURE:
                    String path2 = ImageUtil.getPath(IdentifyUserActivity.this, SelectPictureDialog.cameraUri);
                    Bitmap bitmap2 = ImageUtil.getBitmapFromPath(path2);
                    bg_first.setImageBitmap(bitmap2);
                ...
                .subscribe(new Action1() {
                    ...
                    public void call(Object o) {
                        ...
                        if (bean.isSuccess()) {
                            ...
                            DialogUtils.setDialog(IdentifyUserActivity.this, bean.getError());
                        }
                    }
                }, new Action1<Throwable>() {
                    @Override
