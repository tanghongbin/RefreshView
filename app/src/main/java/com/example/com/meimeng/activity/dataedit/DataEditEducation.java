 ...
 */
public class DataEditEducation extends BaseActivity {
 ...
 * 区别改动区域
 */
        masterHashMap = GetDbUtils.getMsterHashMap(DataEditEducation.this, 1);
    ...
    void suretext() {
        ...
        if (position == -1) {
            ...
            if (isSelectMap.size() == 0) {
                ...
                try{
                    educationid = masterHashMap.get(isSelectMap.get(position));
                }catch (NullPointerException e){
                    e.printStackTrace();
                }finally {
                    educationstr = isSelectMap.get(position);
                }
            }
        }
        ...
        DataEditEducation.this.setResult(8, intent);
        //关闭Activity
        DataEditEducation.this.finish();
    }
