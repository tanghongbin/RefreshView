 ...
 * 婚后生活5
 */
public class HhshActivityFive extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MeiMengApplication.HhshManageActivity.add(this);
        button.setText("提交修改");
...
/**
 * 区别改动区域
 */thistype = 6;//记录是属于哪一类的问题
        questionHashMap = GetDbUtils.getQuestionHashMap(HhshActivityFive.this, thistype);
        //这部分只是界面展示用的
        thisqid = 33;
        question_title.setText("5/5" + questionHashMap.get(thisqid));
        answerHashMap = GetDbUtils.getAnswerHashMap(HhshActivityFive.this, thisqid);
    ...
    void gotonext() {
        ...
        if (!hasselectedbefore) {
            MeiMengApplication.hhshLstUserAnswer.add(lstUserAnswer);
        }
        hasselectedbefore = true;
        IndividualJsonBean individualJsonBean = new IndividualJsonBean();
        individualJsonBean.setLstUserAnswer(MeiMengApplication.hhshLstUserAnswer);
        Gson gson = new Gson();
    ...
    void jumpnext() {
        for (Activity activity : MeiMengApplication.HhshManageActivity) {
            activity.finish();
        }
    }
                    ...
                    .subscribe(new Action1() {
                        ...
                        public void call(Object o) {
                            ...
                            if (basebean.isSuccess()) {
                                Toast.makeText(HhshActivityFive.this, "提交成功", Toast.LENGTH_SHORT).show();
                                for (Activity activity : MeiMengApplication.HhshManageActivity) {
                                    activity.finish();
                                }
                            } else {
                                DialogUtils.setDialog(HhshActivityFive.this, basebean.getError());
                            }
                        }
                        ...
                        public void call(Throwable throwable) {
                            Log.e("HhshActivityFive", throwable.getMessage());
                        }
                    });
        } catch (Exception e) {
            e.printStackTrace();
    ...
    protected void onDestroy() {
        super.onDestroy();
        //将答案从list中移除
        MeiMengApplication.hhshLstUserAnswer.remove(lstUserAnswer);
        MeiMengApplication.HhshManageActivity.remove(this);
    }
}
