 ...
 */
public class DataEditCompanyIndustry extends BaseActivity {
    ...
    private String companyIndustrytr;
 ...
 * 区别改动区域
 */
        masterHashMap = GetDbUtils.getMsterHashMap(DataEditCompanyIndustry.this, 4);
    ...
    void suretext() {
        ...
        if (position == -1) {
            ...
            if (isSelectMap.size() == 0) {
                ...
                try{
                    companyIndustryid = masterHashMap.get(isSelectMap.get(position));
                }catch (NullPointerException e){
                    e.printStackTrace();
                }finally {
                    companyIndustrytr = isSelectMap.get(position);
                }
            }
        }

        Intent intent = new Intent(this, DataEdit.class);
        intent.putExtra("companyIndustry", companyIndustrytr);
        intent.putExtra("companyIndustryid", companyIndustryid);
        setResult(4, intent);
        //关闭Activity
        finish();
    }
