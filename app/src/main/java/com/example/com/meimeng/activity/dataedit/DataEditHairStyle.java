 ...
 */
public class DataEditHairStyle extends BaseActivity {
 ...
 * 区别改动区域
 */
        masterHashMap = GetDbUtils.getMsterHashMap(DataEditHairStyle.this, 19);
    ...
    void suretext() {
        ...
        if (position == -1) {
            ...
            if (isSelectMap.size() == 0) {
                ...
                try{
                    hairStyleid = masterHashMap.get(isSelectMap.get(position));
                }catch (NullPointerException e){
                    e.printStackTrace();
                }finally {
                    hairStyletr = isSelectMap.get(position);
                }
        }}

        Intent intent = new Intent(this, DataEdit.class);
        intent.putExtra("hairStyle", hairStyletr);
