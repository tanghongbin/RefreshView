 ...
 */
public class DataEditFace extends BaseActivity {
 ...
 * 区别改动区域
 */
        masterHashMap = GetDbUtils.getMsterHashMap(DataEditFace.this, 21);
    ...
    void suretext() {
        ...
        if (position == -1) {
            ...
            if (isSelectMap.size() == 0) {
                ...
                try{
                    faceid = masterHashMap.get(isSelectMap.get(position));
                }catch (NullPointerException e){
                    e.printStackTrace();
                }finally {
                    facestr = isSelectMap.get(position);
                }
            }
        }

        Intent intent = new Intent(this, DataEdit.class);
        intent.putExtra("face", facestr);
