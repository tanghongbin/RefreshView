 ...
 */
public class DataEditHairColor extends BaseActivity {
 ...
 * 区别改动区域
 */
        masterHashMap = GetDbUtils.getMsterHashMap(DataEditHairColor.this, 20);
    ...
    void suretext() {
        ...
        if (position == -1) {
            ...
            if (isSelectMap.size() == 0) {
                ...
                try{
                    hairColorid = masterHashMap.get(isSelectMap.get(position));
                }catch (NullPointerException e){
                    e.printStackTrace();
                }finally {
                    hairColorstr = isSelectMap.get(position);
                }
            }
        }

        Intent intent = new Intent(this, DataEdit.class);
        intent.putExtra("hairColor", hairColorstr);
        intent.putExtra("hairColorid", hairColorid);
        setResult(6, intent);
        //关闭Activity
        finish();
    }
