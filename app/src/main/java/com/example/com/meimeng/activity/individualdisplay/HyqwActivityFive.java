 ...
 * 婚姻期望5
 */
public class HyqwActivityFive extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MeiMengApplication.HyqwManageActivity.add(this);
        button.setText("提交修改");
...
/**
 * 区别改动区域
 */thistype = 5;//记录是属于哪一类的问题
        questionHashMap = GetDbUtils.getQuestionHashMap(HyqwActivityFive.this, thistype);
        //这部分只是界面展示用的
        thisqid = 28;
        question_title.setText("5/5" + questionHashMap.get(thisqid));
        answerHashMap = GetDbUtils.getAnswerHashMap(HyqwActivityFive.this, thisqid);
    ...
    void gotonext() {
        ...
        if (!hasselectedbefore) {
            MeiMengApplication.hyqwLstUserAnswer.add(lstUserAnswer);
        }
        hasselectedbefore = true;
        IndividualJsonBean individualJsonBean = new IndividualJsonBean();
        individualJsonBean.setLstUserAnswer(MeiMengApplication.hyqwLstUserAnswer);
        Gson gson = new Gson();
    ...
    void jumpnext() {
        for (Activity activity : MeiMengApplication.HyqwManageActivity) {
            activity.finish();
        }
    }
                    ...
                    .subscribe(new Action1() {
                        ...
                        public void call(Object o) {
                            ...
                            if (basebean.isSuccess()) {
                                Toast.makeText(HyqwActivityFive.this, "提交成功", Toast.LENGTH_SHORT).show();
                                for (Activity activity : MeiMengApplication.HyqwManageActivity) {
                                    activity.finish();
                                }
                            } else {
                                DialogUtils.setDialog(HyqwActivityFive.this, basebean.getError());
                            }
                        }
                        ...
                        public void call(Throwable throwable) {
                            Log.e("HyqwActivityFive", throwable.getMessage());
                        }
                    });
        } catch (Exception e) {
            e.printStackTrace();
    ...
    protected void onDestroy() {
        super.onDestroy();
        //将答案从list中移除
        MeiMengApplication.hyqwLstUserAnswer.remove(lstUserAnswer);
        MeiMengApplication.HyqwManageActivity.remove(this);
    }
